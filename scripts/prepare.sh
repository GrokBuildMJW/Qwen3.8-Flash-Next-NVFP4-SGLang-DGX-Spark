#!/usr/bin/env bash
# Pull the pinned SGLang image, extract the two files that need patching,
# patch them, download the NVFP4 checkpoint as the host user (never root).
set -euo pipefail
...
if ! docker image inspect "${IMAGE}" >/dev/null 2>&1; then
  docker pull "lmsysorg/sglang@${IMAGE_DIGEST}"
  docker tag "lmsysorg/sglang@${IMAGE_DIGEST}" "${IMAGE}"
fi
